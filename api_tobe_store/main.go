...
type TransaksiResponse struct {
    ...
    GambarProduct    string    `json:"gambar_product"`
}
...
func main() {
	db, err := sql.Open("mysql", "praktikum_ToBe:PasswordToBe@tcp(localhost:3306)/praktikum_ToBe_Store")
	if err != nil {
		log.Fatal(err)
	}
	defer db.Close()
...
e.GET("/seller/:kd_seller", func(c echo.Context) error {
	...
	e.GET("/pengiriman_arya_dua", func(c echo.Context) error {
		...
		for rows.Next() {
			...
			err := rows.Scan(&pengiriman.ID, &pengiriman.Kd_pengiriman, &pengiriman.Nama_kurir, &pengiriman.Alamat_tujuan, &pengiriman.Kd_transaksi, &pengiriman.Status_pengiriman, &pengiriman.Ongkir)
			if (err != nil) {
				return err
			}
	...
	e.GET("/pengiriman_by_id", func(c echo.Context) error {
		...
		query := `
			...
			FROM
				pengiriman
			WHERE
				kd_pengiriman = ? AND status_pengiriman = 'Proses'
		`
	
		var pengiriman Pengiriman
	
		err := db.QueryRow(query, kdPengiriman).Scan(&pengiriman.ID, &pengiriman.Kd_pengiriman, &pengiriman.Nama_kurir, &pengiriman.Alamat_tujuan, &pengiriman.Kd_transaksi, &pengiriman.Status_pengiriman, &pengiriman.Ongkir)
		if (err != nil) {
			if err == sql.ErrNoRows {
	...
	e.GET("/transaksi/:kd_user", func(c echo.Context) error {
		// Ambil parameter kd_user dari URL
		kdUser := c.Param("kd_user")
	
		// Query untuk mengambil data transaksi berdasarkan kd_user dengan join tabel
		query := `
			SELECT 
				transaksi.kd_transaksi,
				transaksi.kd_user,
				users.nama_users AS nama_user,
				seller.nama_seller AS nama_seller,
				barang.nama_b AS nama_barang,
				barang.gambar_product,  
				transaksi.jumlah_barang,
				transaksi.total_harga,
				transaksi.tgl_transaksi,
				transaksi.status_pembayaran
			...
			JOIN 
				barang ON transaksi.kd_barang = barang.kd_barang
			WHERE 
				transaksi.kd_user = ?`
	
		// Execute the query
		rows, err := db.Query(query, kdUser)
		if err != nil {
			return err
		}
		defer rows.Close()
	
		var transaksis []TransaksiResponse
		...
		for rows.Next() {
			...
			if err := rows.Scan(
				...
				&t.NamaSeller,
				&t.NamaBarang,
				&t.GambarProduct,  // Scan gambar_product
				&t.JumlahBarang,
				&t.TotalHarga,
				&tglTransaksi,
				&t.StatusPembayaran,
			...
			// Konversi manual dari []uint8 ke time.Time
			t.TglTransaksi, err = time.Parse("2006-01-02 15:04:05", string(tglTransaksi))
			if err != nil {
				return err
			}
	...
	e.POST("/transaksii", func(c echo.Context) error {
		...
		// Mengembalikan kd_transaksi ke klien setelah berhasil membuat transaksi
...
e.GET("/pengirimann_arya_perjalanan", func(c echo.Context) error {
	...
	e.GET("/pengiriman", func(c echo.Context) error {
		// Query untuk mendapatkan semua data pengiriman
		rows, err := db.Query(`
			SELECT
				id,
				kd_pengiriman,
		...
		`)
		if err != nil {
			log.Printf("Kesalahan saat mengambil semua pengiriman dari database: %v\n", err)
			return c.JSON(http.StatusInternalServerError, map[string]string{"error": "Gagal mengambil semua pengiriman"})
		}
		defer rows.Close()
	
		var pengirimanList []Pengiriman
		for rows.Next() {
			var p Pengiriman
			if err := rows.Scan(&p.ID, &p.Kd_pengiriman, &p.Nama_kurir, &p.Alamat_tujuan, &p.Kd_transaksi, &p.Status_pengiriman, &p.Ongkir); err != nil {
				log.Printf("Kesalahan saat memindai pengiriman dari database: %v\n", err)
				return c.JSON(http.StatusInternalServerError, map[string]string{"error": "Gagal mengambil semua pengiriman"})
			}
			pengirimanList = append(pengirimanList, p)
		}
	
		if err := rows.Err(); err != nil {
			log.Printf("Kesalahan saat iterasi pengiriman dari database: %v\n", err)
			return c.JSON(http.StatusInternalServerError, map[string]string{"error": "Gagal mengambil semua pengiriman"})
		}
	
		return c.JSON(http.StatusOK, pengirimanList)
	})	
